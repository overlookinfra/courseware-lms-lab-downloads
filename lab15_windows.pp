class profile::java_app::windows {
  ...
  exec {'install_service':
    ...
    unless   => 'if (!(Get-Service puppet_webapp)){ exit 1 }',
  }
  ...
  exec {'set_args':
    ...
    notify   => Service['puppet_webapp'],
    unless   => "if (!(nssm get puppet_webapp AppParameters| select-string \"${app_location}\" -quiet -SimpleMatch)){ exit 1 }",
  }
  ...
  exec {'set_stdout_log':
    ...
    unless   => "if (!(nssm get puppet_webapp AppStdout| select-string \"${app_base}/stdout.log\" -quiet -SimpleMatch)){ exit 1 }",
  }
  ...
  exec {'set_stderr_log':
    ...
    notify   => Service['puppet_webapp'],
    unless   => "if (!(nssm get puppet_webapp AppStderr| select-string \"${app_base}/stderr.log\" -quiet -SimpleMatch)){ exit 1 }",
  }
